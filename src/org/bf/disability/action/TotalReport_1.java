 ...
 * @author 490058
 */
public class TotalReport_1 extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws SADAREMDBException, SQLException {
        ...
        try {
            ...
            if ("getDetails".equalsIgnoreCase(request.getParameter("mode"))) {
                ...
                empWiseDetails = dao.totalReportCol(ds, districtId, mandalId, villageId, fdate, tdate);
                ...
                if (emp.equals("0")) {
                    ...
                    request.setAttribute("names", name);
                    empWiseDetails = dao.totalReportCol(ds, dist, mandal, village, fdate, tdate);
            ...
            target = "exception";
            int exResult = ExceptionDAO.saveException(ds, sqlEx.toString(), "execute", "TotalReport_1", "DataBase");
            //throw new SADAREMDBException(CommonConstants.DBERROR, sqlEx.getMessage(), CommonConstants.E_ERROR_MESSAGE_DATA_FAILURE, "TotalReport_1", "execute");
        } catch (Exception sqlEx) {
            target = "exception";
            int exResult = ExceptionDAO.saveException(ds, sqlEx.toString(), "execute", "TotalReport_1", "Code");
            //throw new SADAREMDBException(CommonConstants.DBERROR, sqlEx.getMessage(), CommonConstants.E_ERROR_MESSAGE_DATA_FAILURE, "TotalReport_1", "execute");
        }
