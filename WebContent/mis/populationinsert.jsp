   ...
   <tbody>
    <tr>  
    	<td class="<%=style%>" style="border:#337ab7 1px solid ;text-align:center;padding-left:2px;">         
		 ...
		 {%>
		 	<a class='iframe' style="overflow:hidden; font-family:sans-serif" href="<%=request.getContextPath()%>/mispopulation.do?mode=districtWisePopulation&distName=<%=innerList.get(1)%>&district=<%=innerList.get(0)%>&randomid=<%=Math.random()%>">
			      
         <input type="button"  value="Confirmed" class="btn btn-primary my-btn-form" id="" name="">    
         </a>  
         <%}else if( innerList.get(3).equals("N") ||  innerList.get(3).equals("")) { %> 
         <a class='iframe' style="overflow:hidden; font-family:sans-serif" href="<%=request.getContextPath()%>/mispopulation.do?mode=districtWisePopulation&distName=<%=innerList.get(1)%>&district=<%=innerList.get(0)%>&randomid=<%=Math.random()%>">
			      
         <input type="button"  value="Edit" class="btn btn-info my-btn-form" id="" name="">    
         </a>  
         <%} else{%>
         <a class='iframe' style="overflow:hidden; font-family:sans-serif" href="<%=request.getContextPath()%>/mispopulation.do?mode=districtWisePopulation&distName=<%=innerList.get(1)%>&district=<%=innerList.get(0)%>&randomid=<%=Math.random()%>">
			      
         <input type="button"  value="Info" class="btn btn-success my-btn-form" id="" name="">    
         </a> 
         <%} %> 
         </td> 
     </tr>
    ...
    <div class="col-md-4">
		 ...
		 <tr>		 
		 <td style="text-align:center;padding-left:6px;"><input type="button" name="confirm" id="confirm" value="Confirm" class="btn btn-success" <%=ConfirmButtonEnable %> ></td>
		 <td></td>
		 <td><input type="button" name="update" id="update" value="Update" class="btn btn-success" ></td>
