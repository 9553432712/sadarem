...
.panel-heading a.collapsed:after {
  ...
  <script type="text/javascript">
	...
	<%
	 ...
	 //System.out.println(issuependingRecordList); 
   					...
   					else if(process($('#fromdate').val()) > process($('#todate').val()))
					{
   					...
   					{	
   						  ...
   						  document.form_issue.target="_self";
   							document.form_issue.action="<%=request.getContextPath()%>/OpenIssueApprovalSystem.do";
  							...
  							<input type="hidden" id="mode" name="mode" value='pendingIssueReqList'>
									   <div class="row">         
												 ...
												 <div class="input-group" id="fromdatediv" name="fromdatediv" <% if(reqType.equals("P")){ %> style="display:none;" <%} %>>
											      ...
											      <input type="text" class="form-control " id="fromdate" name="fromdate"  style="width:80px;"  autocomplete="off" value=<%=fromdate%>>				
							               </div>
							              </div>	
							              ...
							              <div class=" col-xs-2" > 
											      ...
											      <input type="text" class="form-control " id="todate" name="todate"   style="width:80px;"  autocomplete="off" value=<%=todate%>>				
							               </div>	 
										</div>	 
															  ...
															  <div class=" col-xs-2" > 
														  <button type="button" id="raiseissueButID" class="btn btn-success col-xs-4"><b>Submit</b></button>
										 </div>	
										</div>
										 </form>
 ...
 <%if(issuependingRecordList!=null && issuependingRecordList.size()>0){ %>
  <table id="issueData" class="table table-striped table-bordered" width="90%">
   ...
   <td class="<%=style%>" style="border:#337ab7 1px solid;" >
     <a class='iframe' style="overflow:hidden; font-family:sans-serif" href="<%=request.getContextPath()%>/OpenIssueApprovalSystem.do?mode=getValues&requestid=<%=innerList.get(0)%>&sadaremid=<%=innerList.get(1)%>&reqtype=<%=reqType %>&tkttype=<%=tkttype%>&randomid=<%=Math.random()%>">
        ...
        <td class="<%=style%>" style="border:#337ab7 1px solid;" align="center"><%=innerList.get(4) %> <%if(reqType.equals("P")){ %> day(s)<%} %></td>
         <%if(reqType.equals("A") || reqType.equals("R")){ %>
					...
					$(document).ready(function()
						...
						$(".iframe").colorbox(
								...
								iframe:true, width:"95%", height:"95%",
								 onClosed: function(){document.form_issue.action="<%=request.getContextPath()%>/OpenIssueApprovalSystem.do?";
									document.form_issue.submit();}
								});
