...
mole_two="",pension_type="",pensioncard_no="",parents_marriage="",epiccard_no="",add_proof_type ="",add_proof_id ="",aadhaar_proof_status="",aadhaar_proof_id="";
...
if(PartAformEnteredDataList!=null)
{
	...
	aadhaar_proof_status	 = CommonUtility.getStringFromFileItem((FileItem)PartAformEnteredDataList.get("aadhaar_proof_status"));			// Aadhaar Proof Status
	...
	aadhaar_proof_id		 = CommonUtility.getStringFromFileItem((FileItem)PartAformEnteredDataList.get("aadhaar_proof_id"));					// Aadhaar Proof Id
} 

add_proof_type = "PR008";
...
<%
 ...
 <div class="panel  panel-success">  
				 <div class="panel-heading">
				     <h4 class="panel-title"><b>SADAREM Certificate Part-A Registration Form</b></h4>
				 </div>   
				  <div class="panel-body alert-info">  
				  
 					<form  method="post" name="parta_request_form" id="parta_request_form" enctype="multipart/form-data" >
					...
					<div class="alert alert-warning">
						 <div class="form-inline" style="padding-top:5px;"> 
			                 <div class="input-group col-md-4">
							   ...
							   <%=ComboUtil.createStrComboBoxAuto("camp_id",campList,CommonUtility.checkNullObj(camp_id),"form-control","",true,true,"")%>
			                 </div>
 ...
 <div class="row"> 
 	<div class="alert alert-warning">
 		<div class="form-inline" style="padding-top:5px;">
	 	 	<div class="input-group"> 
		 		<div class="input-group-addon" style="background-color: #37ADB6; color:#fff;text-align:left;"><font color="red">*</font> Aadhaar Proof Status</div>
		 		<select id="aadhaar_proof_status" name="aadhaar_proof_status" class="form-control">
		 			<option value="-1">-Select-</option>
		 			<option value="1" <%if(CommonUtility.checkNullObj(aadhaar_proof_status).equals("1")){%>selected<%} %>>Not Available</option>
		 			<option value="2" <%if(CommonUtility.checkNullObj(aadhaar_proof_status).equals("2")){%>selected<%} %>>Available but not showing in SRDH</option> 
		 		</select>
	 	 	</div>	
 	
	 	 	<div class="input-group">
		 		<div class="input-group-addon" style="background-color: #37ADB6; color:#fff;text-align:left;"  id="aadhaar_id_title_divid">Aadhaar Id</div> 
				<input type="text" class="form-control" id="aadhaar_proof_id" name="aadhaar_proof_id"  value="<%=CommonUtility.checkNullObj(aadhaar_proof_id) %>" maxlength="12" size="15" <%if(!CommonUtility.checkNullObj(aadhaar_proof_status).equals("2")){%>disabled<%} %>>
			</div> 
 	
	 	 	<div class="input-group">
		 		<div class="input-group-addon" style="background-color: #37ADB6; color:#fff;text-align:left;" id="aadhaar_proof_title_divid">Aadhaar Proof</div> 
				<input type="file" class="form-control" id="aadhaar_proof_doc" name="aadhaar_proof_doc" size="15"  <%if(!CommonUtility.checkNullObj(aadhaar_proof_status).equals("2")){%>disabled<%} %>>
			</div> 
 		</div>
 		
 		<div class="form-inline" style="padding-top:5px;text-align: right;">
 			Note : <span class="glyphicon glyphicon-paperclip"></span> File size less than <b>200KB</b> and type <b>JPEG/PDF</b>)
 		</div>
		  ...
		  <table class="table table-striped table-bordered dataTable" style="font-size: 15px; font-weight: bold;"> 
		   ...
		   <tr>
		     ...
		     <tr> 
		       ...
		       <th style="background-color:#337ab7;padding:5px;color:#fff;text-align:center; vertical-align: middle;">Proof Id / File No.</th>
			  ...
			  <tr  style="background-color: #f7f5f1 !important;">
			   <td style="text-align: center; vertical-align: middle"><font color="red">*</font> 1</td> 
		       <td style="text-align:left; vertical-align: middle">
		       		<%=ComboUtil.createStrComboBoxAutoWithAttribute("add_proof_type_disp",proofTypeList, CommonUtility.checkNullObj(add_proof_type), "form-control", "",true,true, ""," disabled")%>
		       		<input type="hidden" id="add_proof_type" name="add_proof_type" value="<%=add_proof_type%>"> 
		       </td> 
...
function fun_reformatDateString(s)
{
	...
	function submitValildations()
	{
		...
		$("#parta_request_form input,select").each( function()
        { 
			    $(this).css({ "background": ""});
        });
		...
		else if($('#reference_form_number').val()=="")
		{
		    ...
		    fun_setbackgroundcolor( $('#reference_form_number'));
		    return false;
		}
		else if($("#form_fill_date").val()=="")
		{
		    ...
		    fun_setbackgroundcolor( $('#form_fill_date'));
		...
		else if($('#mandals').val()==-1)
		{
		    ...
		    $('#mandals').focus();
		    fun_setbackgroundcolor( $('#mandals'));
		    return false;
		}	          
		else if($('#panchayats').val()==-1)
		{
		    ...
		    $('#panchayats').focus();
		    fun_setbackgroundcolor( $('#panchayats'));
		    return false;
		}	          
		else if($('#villages').val()==-1)
		{
		    ...
		    $('#villages').focus();
		    fun_setbackgroundcolor( $('#villages'));
		    return false;
		}
		else if($('#habitation').val()==-1)
		{
		    ...
		    $('#habitation').focus();
		    fun_setbackgroundcolor( $('#habitation'));
		    return false;
		}
		else if($("#house_number").val()=="")
		{
			...
			$("#house_number").focus();
		    fun_setbackgroundcolor( $('#house_number'));
			return false;
		}
		else if($("#pin_code").val()=="")
		{
			...
			$("#pin_code").focus();
		    fun_setbackgroundcolor( $('#pin_code'));
			return false;
		}
		else if($("#phone_no").val()=="")
		{
			...
			$("#phone_no").focus();
		    fun_setbackgroundcolor( $('#phone_no'));
			return false;
		}
		/* else if($("#surname").val()=="")
		{
			...
			$("#surname").focus();
		    fun_setbackgroundcolor( $('#surname'));
			return false;
		} */
		else if($("#first_name").val()=="")
		{
			...
			$("#first_name").focus();
		    fun_setbackgroundcolor( $('#first_name'));
			return false;
		}
		else if($("#full_name_eng").val()=="")
		{
			...
			$("#full_name_eng").focus();
		    fun_setbackgroundcolor( $('#full_name'));
			return false;
		}
		else if($("#date_of_birth").val()=="")
		{
			...
			$("#date_of_birth").focus();
		    fun_setbackgroundcolor( $('#date_of_birth'));
			return false;
		}
		...
		else if($("#grelation").val()=="-1")
		{
			...
			$("#grelation").focus();
		    fun_setbackgroundcolor( $('#grelation'));
			return false;
		}
		else if($("#relation_name").val()=="")
		{
			...
			$("#relation_name").focus();
		    fun_setbackgroundcolor( $('#relation_name'));
			return false;
		}
		else if($("#relation_name_eng").val()=="")
		{
		    ...
		    fun_setbackgroundcolor( $('#relation_name_eng'));
			return false;
		}
		else if($("#education_id").val()=="-1")
		{
			...
			$("#education_id").focus();
		    fun_setbackgroundcolor( $('#education_id'));
			return false;
		}
		else if($("#employment_id").val()=="-1")
		{
			...
			$("#employment_id").focus();
		    fun_setbackgroundcolor( $('#employment_id'));
			return false;
		}
		else if($("#marital_status_id").val()=="-1")
		{
		    ...
		    fun_setbackgroundcolor( $('#marital_status_id'));
			return false;
		}
		else if($("#religion_id").val()=="-1")
		{
			...
			$("#religion_id").focus();
		    fun_setbackgroundcolor( $('#religion_id'));
			return false;
		}
		else if($("#caste_id").val()=="-1")
		{
			...
			$("#caste_id").focus();
		    fun_setbackgroundcolor( $('#caste_id'));
			return false;
		}
		else if(rationStatus>0 && rationStatus<3)
		{ 
			alert("Please enter ration card details");
		    fun_setbackgroundcolor( $('#rationcard_number'));
		    fun_setbackgroundcolor( $('#rationcard_type'));
		    fun_setbackgroundcolor( $('#rationcard_slno'));
			return false;
		}
		else if($("#mole_one").val()=="" || ($("#mole_one").val()).length < 15)
		{
			...
			$("#mole_one").focus();
		    fun_setbackgroundcolor( $('#mole_one'));
			return false;
		} 
		/* else if($("#mole_two").val()=="" || ($("#mole_two").val()).length < 15)
		{
			...
			$("#mole_two").focus();
		    fun_setbackgroundcolor( $('#mole_two'));
			return false;
		}  */
		...
		else if($("#pension_type").val()!="-1" && $("#pensioncard_no").val()=="")
		{
		    ...
		    fun_setbackgroundcolor( $('#pensioncard_no'));
			return false;					
		}
		else if($("#parents_marriage").val()=="-1")
		{
		    ...
		    fun_setbackgroundcolor( $('#parents_marriage'));
			return false;
		}
		else if($('#profile_pic').val()=="" || $('#profile_pic').val()==null || ($('#profile_pic').val()).length ==0 )
		{
			...
			$('#profile_pic').focus();
		    fun_setbackgroundcolor( $('#profile_pic'));
			return false;
		}  
		else if($("#aadhaar_proof_status").val()=="-1")
		{
			alert("Please select Aadhaar proof status");
			$("#aadhaar_proof_status").focus();
			fun_setbackgroundcolor($("#aadhaar_proof_status"));
			return false;
		} 
		else if($("#aadhaar_proof_status").val()=="2" && ( $("#aadhaar_proof_id").val()=="" || fun_validateAadhaarID($("#aadhaar_proof_id").val())==false ) )
		{
			alert("Please enter valid Aadhaar Id");
			$("#aadhaar_proof_id").val("");
			$("#aadhaar_proof_id").focus();
			fun_setbackgroundcolor($("#aadhaar_proof_id"));
			return false;
		}
		else if($("#aadhaar_proof_status").val()!="-1" && $("#aadhaar_proof_status").val()=="2" && ($('#aadhaar_proof_doc').val()=="" || $('#aadhaar_proof_doc').val()==null || ($('#aadhaar_proof_doc').val()).length == 0))
		{ 
			alert("Please upload Adhaar document (Image less than 200KB jpeg/jpg/png type)");
			$('#aadhaar_proof_doc').focus();
		    fun_setbackgroundcolor( $('#aadhaar_proof_doc'));
			return false;
		}
		else if(addProofValid>0 && addProofValid<3)
		{
			alert("Please enter valid additional document details");
		    fun_setbackgroundcolor( $('#add_proof_id'));
		    fun_setbackgroundcolor( $('#add_proof_doc'));
			return false;
		}
		else if( $("#add_proof_id").val()!="" &&  $("#add_proof_id").val()==$("#aadhaar_proof_id").val())
		{
	...
	function fun_setbackgroundcolor(id)
	{
		$(id).css({ "background": "#FFCECE"});
	}
...
$(document).ready( function()
{
	...
	$('body').on('focusin focusout change blur keyup','#relation_name_eng',function()
		...
		$("#aadhaar_proof_status").change(function()
		{
			if($(this).val()=="-1" || $(this).val()=="1")
				{
					$("#aadhaar_proof_id").prop('disabled', true);
					$("#aadhaar_id_title_divid").html(" Aadhaar Id");
					$("#aadhaar_proof_id").val("");
					$("#aadhaar_proof_doc").prop('disabled', true);
					$("#aadhaar_proof_title_divid").html(" Aadhaar Proof");
					$("#aadhaar_proof_doc").val("");
				} 
				else
				{
					$("#aadhaar_proof_id").prop('disabled', false);
					$("#aadhaar_id_title_divid").html("<font color='red'>*</font> Aadhaar Id");
					$("#aadhaar_proof_doc").prop('disabled', false);
					$("#aadhaar_proof_title_divid").html("<font color='red'>*</font> Aadhaar Proof");
				}
		});
				...
				{	 
				    ...
				    else	if(confirm("Are you sure you want to submit the request?"))
					{
						...
						$("#aadhaar_proof_doc").prop('disabled', false);
							 	...
							 	document.parta_request_form.target="_self";
							 	document.parta_request_form.action="<%=request.getContextPath()%>/withoutproofparta.do?mode=logpartawithoutproofreqform&randomid="+Math.random();
							 	document.parta_request_form.submit();
