...
<title>:: SADAREM :: R1.3 Report</title>
...
String selDist		=	CommonUtility.checkNullObj(request.getAttribute("district"));
String selMandal	=	CommonUtility.checkNullObj(request.getAttribute("mandal"));
String selVillage 	= 	CommonUtility.checkNullObj(request.getAttribute("village"));
String selAreatype	=	CommonUtility.checkNullObj(request.getAttribute("areatype1"));
...
String header		= CommonUtility.checkNullObj(request.getAttribute("sadaremnothavingheadingmsg"));
...
$(document).ready(function()
    ...
    $('body').on('change','#district1',function()
    		{	
    	postRequest("<%=request.getContextPath()%>/CommonReportsAjax.do?action=loadMandalsbyurbanruralareawise&mandalidname=mandal&distId="+ $('#district1').val()+"&areatype="+ $('#areatype1').val()+"&randomid="+Math.random(),"mandalSPANID");
    		});
    ...
    $('body').on('change','#mandal',function()
    	...
    	$("#sadaremnothavingaadharformSubmitbut").click(function( event )
    			{
				...
				else if($("#mandal").val()=="-1")
				{
					...
					return false;
    					...
    					else
    					{
    						...
    						document.districtwiseform.action="<%=request.getContextPath()%>/newSadaremNotHavingAadhar.do?randomid="+Math.random();
...
<body>
				    ...
				    <div class="panel  panel-primary" id="districtwiseform">
				        <div class="panel-heading" data-toggle="collapse" data-target="#collapseTwo" href="#collapseTwo" class="collapsed" style="cursor: pointer;text-align:center;">
				             <h4 class="panel-title">
				          			R1.3 : SADAREM ID Not Tagged To Aadhar Card No's
				      		</h4>
				        </div>
				            		...
				            		<form class="form-inline" role="form" id="districtwiseform" name="districtwiseform" method="post">
					            		<input type="hidden" id="mode" name="mode" value="getData">
					            		 ...
					            		 <div class="input-group " id="Trigpdiv1" >
											      <div class="input-group-addon" style="background-color: #eee; color:#000;font-weight:bold;">Project<font color="red">*</font></div>										  
							                ...
							                <div class="input-group " id="areatypediv1" >
											      <div class="input-group-addon" style="background-color: #eee; color:#000;font-weight:bold;">Area Type<font color="red">*</font></div>										  
				                             <select class="form-control mycomboStyle " id='areatype1' name='areatype1'>
				                             <option value='-1' <%if(selAreatype.equalsIgnoreCase("-1")) {%> selected <%} %>>-Select-</option>
				                              <option value='U' <%if(selAreatype.equalsIgnoreCase("U")) {%> selected <%} %>>Urban</option>
				                               <option value='R' <%if(selAreatype.equalsIgnoreCase("R")) {%> selected <%} %>>Rural</option>
				                             </select> 
				                                </div> 		
											
										 	<div class="input-group" >
										      <div class="input-group-addon" style="background-color: #eee; color:#000;font-weight:bold;">District<font color="red">*</font></div>
										    <span id="districtSPANID1"> <%=ComboUtil.createStrComboBoxAuto("district1", districtList,selDist,"form-control mycomboStyle","",true,true,"")%></span>	
										 </div>
										  ...
										  <div class="input-group " id="mandaldiv">
										     <div class="input-group-addon" style="background-color: #eee; color:#000; font-weight:bold;">Mandal<font color="red">*</font></div>
										      <span id="mandalSPANID"><%=ComboUtil.createStrComboBoxAuto("mandal",mandalList,selMandal,"form-control mycomboStyle","",true,true,"")%></span>
										  </div>
										  <div class="input-group" id="villagediv">											    
										   <div class="input-group-addon" style="background-color: #eee; color:#000; font-weight:bold;">Village</div>
										    <span id="villageSPANID"><%=ComboUtil.createStrComboBoxAuto("village",villageList,selVillage,"form-control mycomboStyle","",true,false,"")%></span>
										  </div>
											
										   
							               
										
										 <button type="button" id="sadaremnothavingaadharformSubmitbut" class="btn btn-success"><b>Submit</b></button>
 ...
 <%=header%>
<a href="<%=request.getContextPath()%>/newSadaremNotHavingAadhar.xls?mode=excelNew&district=<%=selDist%>&mandal=<%=selMandal%>&village=<%=selVillage%>&heading=<%=header%>&areatype=<%=selAreatype%>&Trigpsel=<%=Trigpsel %> ">
