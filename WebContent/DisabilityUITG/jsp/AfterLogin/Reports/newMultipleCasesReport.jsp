...
<title>:: SADAREM ::R4.4 Report</title>
...
String caste=CommonUtility.checkNullObj((String)request.getAttribute("caste"));
String education=CommonUtility.checkNullObj((String)request.getAttribute("education"));
String fromage=CommonUtility.checkNullObj((String)request.getAttribute("fromage"));
String toage=CommonUtility.checkNullObj((String)request.getAttribute("toage"));
String fromdisbpercent=CommonUtility.checkNullObj((String)request.getAttribute("fromdisbpercent"));
String todisbpercent=CommonUtility.checkNullObj((String)request.getAttribute("todisbpercent"));
String employment=CommonUtility.checkNullObj((String)request.getAttribute("employment"));
String disability=CommonUtility.checkNullObj((String)request.getAttribute("disability"));
String gender=CommonUtility.checkNullObj((String)request.getAttribute("gender"));
...
$('#todate1').datetimepicker(
    ...
    $('#areatype1').change(function()
   		 {	
   			  postRequest("<%=request.getContextPath()%>/CommonReportsAjax.do?action=loaddistrictss&areatype="+ $('#areatype1').val()+"&randomid="+Math.random(),"districtSPANID1");
    	...
    	$("#districtwiseformSubmitbut").click(function( event )
    			{
    				...
    				if($("#fromdate1").val()=="")
    					...
    					else if(!($("#fromage").val()>=0 && $("#fromage").val()<=100))
    					{
    						alert("Please provide Valid Minimum age(0-100)");
    						$("#fromage").focus();
    					...
    					else if(!($("#toage").val()>=0 && $("#toage").val()<=100))
    					{
    						alert("Please provide Valid Maximum age(0-100)");
    						$("#toage").focus();
    					...
    					{
    						alert("Please provide Valid age(0-100)");
    						$("#fromage").focus();
    					...
    					else if($("#fromdisbpercent").val()=="")
    					{
    						alert("Please select Minimum Disability Percentage(%)");
    						$("#fromdisbpercent").focus();
							
							 event.preventDefault();
							event.stopPropagation();
							return false;
    					}
    					else if($("#todisbpercent").val()=="")
    					{
    						alert("Please select Maximum Disability Percentage(%)");
    						$("#todisbpercent").focus();
    					...
    					else if(!($("#fromdisbpercent").val()>=0 && $("#fromdisbpercent").val()<=100))
    					{
    						alert("Please provide Valid Minimum Disability percentage(0-100)");
    						$("#fromdisbpercent").focus();
    					...
    					else if(!($("#todisbpercent").val()>=0 && $("#todisbpercent").val()<=100))
    					{
    						alert("Please provide Valid Maximum Disability percentage(0-100)");
    						$("#todisbpercent").focus();
    					...
    					else if(($("#fromdisbpercent").val()>$("#todisbpercent").val()))
    					{
    						alert("Please provide Valid  Disability percentage(0-100)");
    						$("#fromdisbpercent").focus();
    					...
    					else
    					{
    						...
    						document.districtwiseform.action="<%=request.getContextPath()%>/newMultipleCasesReport.do?randomid="+Math.random();
				    ...
				    <div class="panel  panel-primary" id="districtwiseform">
				        <div class="panel-heading" data-toggle="collapse" data-target="#collapseTwo" href="#collapseTwo" class="collapsed" style="cursor: pointer;text-align:center;">
				             <h4 class="panel-title">
				          			<b> R4.2 Search by Age/Gender/Disability/Caste/Employment/Qualification</b>
				      		</h4>
				        </div>
				            		...
				            		<form class="form-inline" role="form" id="districtwiseform" name="districtwiseform" method="post">
					            		<input type="hidden" id="mode" name="mode" value="getData">
							                ...
							                <div class="input-group " id="areatypediv1" >
				                             ...
				                             <option value='-1' <%if(areatype1.equalsIgnoreCase("-1")){ %>selected<%} %>>-Select-</option>
										    ...
										    <span id="districtSPANID1"> <%=ComboUtil.createStrComboBoxAuto("district1", districtList,district1,"form-control mycomboStyle","",true,true,"")%></span>	
										 </div>
										  ...
										  <div class="input-group " id="mandaldiv">
										   ...
										   <div class="input-group" id="castediv">											    
										   <div class="input-group-addon" style="background-color: #eee; color:#000; font-weight:bold;">Caste</div>
										    <span id="casteSPANID"><%=ComboUtil.createStrComboBoxAuto("caste",casteList,caste,"form-control mycomboStyle","",true,false,"")%></span>
										  </div> 
										     <div class="input-group " id="genderdiv" >
											      <div class="input-group-addon" style="background-color: #eee; color:#000;font-weight:bold;">Gender</div>										  
				                             <select class="form-control mycomboStyle " id='gender' name='gender'>
				                             <option value='-1' <%if(gender.equalsIgnoreCase("-1")){ %>selected<%} %>>All</option>
										   ...
										   <div class="input-group" id="educationdiv">											    
										   <div class="input-group-addon" style="background-color: #eee; color:#000; font-weight:bold;">Qualification</div>
										    <span id="educationSPANID"><%=ComboUtil.createStrComboBoxAuto("education",educationList,education,"form-control mycomboStyle","",true,false,"")%></span>
										  </div> 
										    <div class="input-group" id="employmentdiv">											    
										   <div class="input-group-addon" style="background-color: #eee; color:#000; font-weight:bold;">Employment</div>
										    <span id="emplpoymentSPANID"><%=ComboUtil.createStrComboBoxAuto("employment",employmentList,employment,"form-control mycomboStyle","",true,false,"")%></span>
										  </div> 
										   <div class="input-group " id="agediv" style="width: 190px;">
											      <div class="input-group-addon" style="background-color: #eee; color:#000;font-weight:bold;">Age</div>
											      <input type="text" class="form-control" id="fromage" name="fromage" onkeypress="return NumbersOnly(event);" autocomplete="off" maxlength="3" value=<%=fromage%>>   <span class="input-group-addon" style="background-color: #eee; color:#000;font-weight:bold;">to</span>
											      <input type="text" class="form-control" id="toage"  name="toage"  onkeypress="return NumbersOnly(event);"  autocomplete="off" maxlength="3" value=<%=toage%>>
											     
							               </div>
							                <div class="input-group" id="disablediv">											    
										   <div class="input-group-addon" style="background-color: #eee; color:#000; font-weight:bold;">Disability</div>
										    <span id="disablilitySPANID"><%=ComboUtil.createStrComboBoxAuto("disability",disabilityList,disability,"form-control mycomboStyle","",true,false,"")%></span>
										  </div>
	
							                <div class="input-group " id="fromdisbpercentdiv" style="width: 260px;">
											      <div class="input-group-addon" style="background-color: #eee; color:#000;font-weight:bold;">Disability(in %)</div>
											       <input type="text" class="form-control" id="fromdisbpercent" name="fromdisbpercent" onkeypress="return NumbersOnly(event);" autocomplete="off" maxlength="3" value=<%=fromdisbpercent%>>   <span class="input-group-addon" style="background-color: #eee; color:#000;font-weight:bold;">to</span>
											      <input type="text" class="form-control" id="todisbpercent" name="todisbpercent" onkeypress="return NumbersOnly(event);" autocomplete="off" maxlength="3" value=<%=todisbpercent%>>
											   
							               </div><br>
										  
										  
										 <button type="button" id="districtwiseformSubmitbut" class="btn btn-success" Style="margin-left:auto;margin-right:auto;display:block;"><b>Submit</b></button>
...
<a href="<%=request.getContextPath()%>/newMultipleCasesReport.xls?mode=excelNew&fromdate=<%=fromdate1%>&todate=<%=todate1%>&district=<%=district1%>&mandal=<%=mandal1%>&village=<%=village1%>&areatype=<%=areatype1%>&caste=<%=caste%>&gender=<%=gender%>&disability=<%=disability%>&education=<%=education%>&fromage=<%=fromage%>&toage=<%=toage%>&fromdisbpercent=<%=fromdisbpercent%> &todisbpercent=<%=todisbpercent%>&employment=<%=employment%>&heading=<%=heading %>&TRIGPSel=<%=TRIGPSel %>  ">
<img height="35" width="35" src="<%=request.getContextPath()%>/images/excel.jpg" border="0" align="right" title="Export Excel"/>
 </a>                  </td>
                    </tr>
                </table>
<%--  <div <%if(fromdate1.length()==10 && todate1.length()==10 && !district1.equals("-1") && !mandal1.equals("-1") && !village1.equals("-1")){ %>style="width:95%;margin:0px auto;" <%}else{ %> style="width:65%;margin:0px auto;" <% }%>>
 ...
 <%=heading%>
 <a href="<%=request.getContextPath()%>/newCertiUploadReport.xls?mode=excelNew&fromdate=<%=fromdate1%>&todate=<%=todate1%>&district=<%=district1%>&mandal=<%=mandal1%>&village=<%=village1%>&heading=<%=heading%>&areatype=<%=areatype1%> ">
    		 ...
    		 }else
    		 {
    			 classStyle="firstrow";
                  ...
                  %>      <tr>
                            ...
                            <td class="<%=classStyle%>" style="text-align: center">
                              <%=innerList.get(5) %><%=innerList.get(6) %>
                            </td>
