<%-- 
    Document   : MedicalInterventionHI
    Created on : Jan 4, 2011, 9:53:26 AM
    Author     : 509865
--%>
                ...
                <td height="445" align="center" valign="top">
                    ...
                    <logic:present name="medicalInterventionHIList" scope="request">
                        <table  align="center" cellspacing="1" border="1" cellpadding="4" class="table" width="90%">
                            <tr><th class="formhdbg" align="center" colspan="8">Medical Intervention HI Report</th></tr>
                            <tr>
                        ...
                        <table align="center">
                            <%
                                        session.setAttribute("medicalInterventionHIList", medicalInterventionHIList);
                            %>
                            <a href="medicalInterventionHIExcel.xls?fdate=<%=fromdate%>&todate=<%=todate%>&dName=<%=dName%>&mName=<%=mName%>&vName=<%=vName%>&hName=<%=hName%>&dID=<%=district_id%>&mID=<%=mandal_id%>&vID=<%=village_id%>&hID=<%=habitation_id%>" target="_blank">
                                Export To Excel <img src="DisabilityUITG/images/excel.jpg"
                                                     height="25" width="25"/></a> &nbsp; &nbsp; &nbsp;
                            <a href="medicalInterventionHIPrint.xls?fdate=<%=fromdate%>&todate=<%=todate%>&dName=<%=dName%>&mName=<%=mName%>&vName=<%=vName%>&hName=<%=hName%>&dID=<%=district_id%>&mID=<%=mandal_id%>&vID=<%=village_id%>&hID=<%=habitation_id%>" target="_blank">
