<%-- 
    Document   : OtherNeedsVI
    Created on : Jan 4, 2011, 9:36:19 AM
    Author     : 509865
--%>
...
<%          String fromdate = (String) request.getParameter("fromdate");
            ...
            Iterator iter = otherNeedsVIList.iterator();
            while (iter.hasNext()) {
                ...
                <td height="445" align="center"  valign="top">
                    ...
                    <logic:present name="otherNeedsVIList" scope="request">
                        <table  align="center" cellspacing="1" border="1" cellpadding="4" class="table" width="40%">
                            <tr><th class="formhdbg" align="center" colspan="3">Other Needs VI Report</th></tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="formhdbg" align="center"><%=otherNeedsVITotal %></td>

                            </tr>
                        </table><br>
                        <table align="center">
                            <%
                                        session.setAttribute("otherNeedsVIList", otherNeedsVIList);
                            %>
                            <a href="otherNeedsVIExcel.xls?fdate=<%=fromdate%>&todate=<%=todate%>&dName=<%=dName%>&mName=<%=mName%>&vName=<%=vName%>&hName=<%=hName%>&dID=<%=district_id%>&mID=<%=mandal_id%>&vID=<%=village_id%>&hID=<%=habitation_id%>&D=<%=typeOfDisability%>" target="_blank">
                                Export To Excel <img src="DisabilityUITG/images/excel.jpg"
                                                     height="25" width="25"/></a> &nbsp; &nbsp; &nbsp;
                            <a href="otherNeedsVIPrint.xls?fdate=<%=fromdate%>&todate=<%=todate%>&dName=<%=dName%>&mName=<%=mName%>&vName=<%=vName%>&hName=<%=hName%>&dID=<%=district_id%>&mID=<%=mandal_id%>&vID=<%=village_id%>&hID=<%=habitation_id%>&D=<%=typeOfDisability%>" target="_blank">
