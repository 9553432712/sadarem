<%-- 
    Document   : AssistiveDevicesADLEquipments
    Created on : Dec 27, 2010, 9:37:22 AM
    Author     : 509865
--%>
                ...
                <td height="445" align="center"  valign="top">
                    ...
                    <logic:present name="adlEquipmentsList" scope="request">
                        <table  align="center" cellspacing="1" border="1" cellpadding="4" class="table" width="90%">
                            <tr><th class="formhdbg" align="center" colspan="11">Assistive Devices ADL Equipments Report</th></tr>
                            <tr>
                        ...
                        <table align="center">
                            <%
                                        session.setAttribute("adlEquipmentsList", adlEquipmentsList);
                            %>
                            <a href="AssistiveDevicesADLEquipmentsListOHExcel.xls?fdate=<%=fromdate%>&todate=<%=todate%>&dName=<%=dName%>&mName=<%=mName%>&vName=<%=vName%>&hName=<%=hName%>&dID=<%=district_id%>&mID=<%=mandal_id%>&vID=<%=village_id%>&hID=<%=habitation_id%>" target="_blank">
                                Export To Excel <img src="DisabilityUITG/images/excel.jpg"
                                                     height="25" width="25"/></a> &nbsp; &nbsp; &nbsp;
                            <a href="AssistiveDevicesADLEquipmentsListOHPrint.xls?fdate=<%=fromdate%>&todate=<%=todate%>&dName=<%=dName%>&mName=<%=mName%>&vName=<%=vName%>&hName=<%=hName%>&dID=<%=district_id%>&mID=<%=mandal_id%>&vID=<%=village_id%>&hID=<%=habitation_id%>" target="_blank">
