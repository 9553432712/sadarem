<%-- 
    Document   : GeneralNeedsPersonalReport
    Created on : Feb 14, 2011, 5:49:13 PM
    Author     : 509865
--%>
...
<html:html>
    ...
    <head>
        <%
                    ...
                    String total = (String)request.getParameter("count");
        ...
        <body  onload="changecolor('<%=prenext%>')">
            <div style="width:900px; height:100%; overflow-x:scroll;
                ...
                <table  border="0" align="center" cellspacing="1" cellpadding="4" class="inputform" width="100%">
                    ...
                    <logic:iterate id="details" name="genaralPersonalList"  scope="request">
                        <%if (i % 2 == 1) {%>
                        <tr>
                            ...
                            <% if(cValue.equals("1")) { %>
                            <td class="formbg"><bean:write name="details" property="surgeryType"/></td> <% } %>
                ...
                <table align="center">
                    ...
                    <a href="personalGeneralNeedsTotal.xls?getpersonalGeneralNeedsTotal=getpersonalGeneralNeedsTotal&fD=<%=fD%>&tD=<%=tD%>&dI=<%=dI%>&mI=<%=mI%>&vI=<%=vI%>&hI=<%=hI%>&R=<%=cValue%>&cName=<%=cName%>&tN=<%=tableName%>&D=<%=D%>&count=<%=total%>&b=<%=subRequired%>"  target="_blank">Total
                        <img src="DisabilityUITG/images/excel.jpg" height="25" width="25"/></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="personalGeneralNeedsPagewiseExcel.xls?fD=<%=fD%>&tD=<%=tD%>&dI=<%=dI%>&mI=<%=mI%>&vI=<%=vI%>&hI=<%=hI%>&R=<%=cValue%>&c=<%=c%>&s=<%=s%>&b=<%=subRequired%>&D=<%=D%>"  target="_blank">Individual Page
                        <img src="DisabilityUITG/images/excel.jpg" height="25" width="25"/></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

                    <br>
                    <logic:notEqual  name="removeback" value="yes" scope="request">
                        <a  href="<%=url + "&back=true&nexxt=false" + "&cName=" + cName + "&cValue=" + cValue + "&tN=" + tableName%>" style="color:black"><<=Previous</a>
                    </logic:notEqual>
                    <logic:notEqual  name="removeprevious" value="yes" scope="request">
                        <a  href="<%=url + "&nexxt=false&back=false&start=" + preprev + "&cName=" + cName + "&cValue=" + cValue + "&tN=" + tableName%>" style="color:green">Previous</a>
                    </logic:notEqual>
                    <%
                    ...
                    <a  href="<%=url + "&nexxt=false&back=false&start=" + start + "&cName=" + cName + "&cValue=" + cValue + "&tN=" + tableName%>" id="<%=idvar%>" onclick="changecolor('<%=idvar%>')" style="color:blue"><%out.println(start);%></a>
                    <% ++idvar;
                            }%>
                    <logic:equal  name="removenext" value="no" scope="request">
                        <a  href="<%=url + "&nexxt=false&back=false&start=" + prenextt + "&cName=" + cName + "&cValue=" + cValue + "&tN=" + tableName%>" style="color:green">Next</a>
                    </logic:equal>
                    <logic:notEqual  name="removenexxt" value="yes" scope="request">
                        <a  href="<%=url + "&nexxt=true&back=false" + "&cName=" + cName + "&cValue=" + cValue + "&tN=" + tableName%>" style="color:black">Next=>></a>
                    </logic:notEqual>
                </table>
            </div>
        </body>
