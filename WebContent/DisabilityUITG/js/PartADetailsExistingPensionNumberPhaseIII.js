...
function getNonpensionerCampDetails() {
   ...
   var url="MemberDetails.do?mode=campdropdownList&disabilityid="+disabilityId;
    multipleDisabilityRequest.open("GET",url,true)
    multipleDisabilityRequest.send(null);
}
...
function getNonpensionerCampdateDetails(partaCampId) {
   ...
   var url="MemberDetails.do?mode=campdatesdropdownList&disabilityid="+partaCampId;
    multipleDisabilityRequest.open("GET",url,true)
    multipleDisabilityRequest.send(null);
}
...
function  createCampObjectslist()
{
    ...
    var url="MemberDetails.do?mode=getCampDetails&disabilityid="+disabilityId;
    x.open("GET",url,true)
    x.send();
}
...
{ 
    ...
    if(x.readyState==4 || x.readyState=="complete")
    {
        ...
        while(z<=counts)
        {
            ...
            addoptions(rs1,rs2,"partaCampId");
            z++;
        }
    }
}
...
function  createCampDateObjectLists(partaCampId)
{
    ...
    var url="MemberDetails.do?mode=getCampDetails&disabilityid="+partaCampId;
    x.open("GET",url,true)
    x.send();
}
function getCampDateDetailssLists()
{
    ...
    if(x.readyState==4 || x.readyState=="complete")
    {
        ...
        while(z<=counts)
        {
            rs1=m.getElementsByTagName("name")[z].childNodes[0].nodeValue;
            rs2=m.getElementsByTagName("id")[z].childNodes[0].nodeValue;
            addoptions(rs1,rs2,"partaCampDate");
            z++;
        }
    }
}
...
function  createCampObject()
{
    ...
    var url="insertRationCardPersonalDetails.do?mode=campdropdownList&disabilityid="+disabilityId;
    x.open("GET",url,true)
    x.send();
}
...
function  createCampDateObject()
{
    ...
    var url="insertRationCardPersonalDetails.do?mode=campdatesdropdownList&disabilityid="+partaCampId;
    x.open("GET",url,true)
    x.send();
}
...
function addoptions(result1,result2,name)
{
