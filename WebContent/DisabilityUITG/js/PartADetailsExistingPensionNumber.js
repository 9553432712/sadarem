...
function  createCampObject()
{
    ...
    var url="insertPartAaction.do?insertPersonalDetails=getCampDetails&disabilityid="+disabilityId;
    x.open("GET",url,true)
    x.send();
}
...
function  createCampDateObject()
{
    ...
    var url="insertPartAaction.do?insertPersonalDetails=getCampDateDetails&disabilityid="+partaCampId;
    x.open("GET",url,true)
    x.send();
}
function getCampDateDetailss()
{
    ...
    if(x.readyState==4 || x.readyState=="complete")
    {
        ...
        while(z<=counts)
        {
            rs1=m.getElementsByTagName("name")[z].childNodes[0].nodeValue;
            rs2=m.getElementsByTagName("id")[z].childNodes[0].nodeValue;
            addoption(rs1,rs2,"partaCampDate");
            z++;
        }
    }
}
...
function phoneNo()
{
    ...
    if(phoneno != null)
    {
        ...
        if(phoneno.value.match(phonenoReg)) 
        {
          return true;
        }
        else
