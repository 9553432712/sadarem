...
function personStatus(){
    ...
    for (counter = 0; counter < NewCertificateForm.personstatus.length; counter++)
    {
        if (NewCertificateForm.personstatus[counter].checked)
            radio_choice = true;
    }
    if (!radio_choice)
    {
...
{
    if(document.NewCertificateForm.epiccard.checked)
    {
...
function validateemailAddress(){
    var address = document.forms[0].emailId;
    if(address != null){
        if(address.value!=""){
...
{
    if(document.NewCertificateForm.pensioncard.checked)
    {
